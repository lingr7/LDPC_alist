...
def _bitsandnodes(H):
    ...
    if type(H) != scipy.sparse.csr_matrix:
        bits_indices, bits = np.where(H)#这里只返回非0元素索引，两个数组，记录坐标
        nodes_indices, nodes = np.where(H.T)#其实直接 nodes_indices = bits, nodes = bits_indices也可以达到类似的效果，只不过虽然记录下坐标，但是顺序不对
    else:
        bits_indices, bits = scipy.sparse.find(H)[:2]
        nodes_indices, nodes = scipy.sparse.find(H.T)[:2]
    bits_histogram = np.bincount(bits_indices)#基本作用是记录数组中出现的值的频数，从低到高排列
    nodes_histogram = np.bincount(nodes_indices)#但因为传递的是索引，某个出现的索引出现的次数，就是这一行的1的个数，或者这一行的列的个数。这是一个记录行重列重的方法。
